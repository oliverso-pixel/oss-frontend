// src/app/(protected)/users/[userId]/followers/page.tsx
"use client";

import { useEffect } from 'react';
import { useParams, useRouter } from 'next/navigation';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import { fetchFollowers, fetchUserProfile } from '@/store/socialSlice';
...
export default function UserFollowersPage() {
  ...
  const { followers, status, userProfile } = useAppSelector((state) => state.social);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-6">{profileName}的關注者</h1>
      ...
      {status === 'succeeded' && followers.length === 0 && (
        <div className="text-center py-12 border-2 border-dashed rounded-lg">
            ...
            <h3 className="mt-2 text-sm font-medium">沒有關注者</h3>
            <p className="mt-1 text-sm text-gray-500">這位用戶目前沒有任何關注者。</p>
        </div>
      )}
